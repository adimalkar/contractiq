/**
 * WebSocket client manager for Termnova real-time streaming and notifications.
 */
  ...
  connectNotifications(onMessageCallback) {
    ...
    try {
      ...
      this.notifSocket.onopen = () => {
        ...
        this.reconnectAttempts = 0;
        console.log("[Termnova WS] Connected to push notification stream");
      };
      ...
      this.notifSocket.onmessage = (event) => {
        try {
          ...
          console.debug("[Termnova WS] Non-JSON payload", event.data);
        }
      };
      ...
      console.warn("[Termnova WS] Notification socket init failed", err);
    }
  }
}
