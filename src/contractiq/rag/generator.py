...
from contractiq.rag import Citation, GeneratedAnswer, GradedChunk

logger = structlog.get_logger(__name__)

SYSTEM_PROMPT = """You are ContractIQ, an elite enterprise AI contract intelligence assistant.
...
class AnswerGenerator:
    ...
    async def generate_stream(
        ...
        try:
            ...
            kwargs: dict[str, Any] = {"temperature": 0.1, "max_tokens": 800, "stream": True}
            if self.provider == "openai" and self.settings.OPENAI_API_KEY:
                kwargs["api_key"] = self.settings.OPENAI_API_KEY
            elif self.provider == "bedrock":
                kwargs["aws_region_name"] = self.settings.AWS_REGION
